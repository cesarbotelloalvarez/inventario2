...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const articulo = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.articulo.update({ where: { id }, data: { status: "ASIGNADO", trabajadorActualId: body.trabajadorId, fechaSalida: new Date() } });
      await registrarMovimiento(tx, { articuloId: id, trabajadorId: body.trabajadorId, tipo: "ASIGNACION", statusAnterior: current.status, statusNuevo: updated.status, condicionAnterior: current.condicion, condicionNueva: updated.condicion, comentarios: body.comentarios });
