...
export default async function FueraInventarioPage() {
  const articulos = isDemoMode
    ? demoArticulos.filter((a) => !a.oculto && a.status === "PRESTADO")
    : await prisma.articulo.findMany({
        where: { oculto: false, status: "PRESTADO" },
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">Serie: {a.numeroSerie ?? "—"}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <StatusBadge status={a.status} />
                    <CondicionBadge condicion={a.condicion} />
                    <span className="text-sm text-slate-600">{a.trabajadorActual?.nombre ?? "Sin trabajador"}</span>
                    <span className="text-sm font-medium">{formatDiasDesde(a.fechaSalida)}</span>
                  </div>
                </div>
              </div>
              <div className="min-w-80"><DevolucionForm articuloId={a.id} /></div>
            </div>
          </Card>
        ))}
